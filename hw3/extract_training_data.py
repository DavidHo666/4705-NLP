...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        result = []
        self.__generate_index(pos, words, state.stack, result)
        self.__generate_index(pos, words, state.buffer, result)
        return np.array(result)

    def __generate_index(self, pos, words, container, result):
        for i in range(3):
            try:
                idx = container[-i-1]
                if pos[idx] in ['CD', 'NNP', 'UNK', 'NULL', None]:
                    vocab = '<' + pos[idx] + '>' if pos[idx] else '<ROOT>'
                    result.append(self.word_vocab[vocab])
                else:
                    result.append(self.word_vocab[words[idx].lower()] if words[idx].lower() in self.word_vocab.keys()
    ...
    def get_output_representation(self, output_pair):  
        ...
        # outputs_generator = self.__generate_outputs()
        # for idx, item in enumerate(outputs_generator):
        #     if output_pair == item:
        #         return keras.utils.to_categorical([idx], num_classes=91)
        #
        # if output_pair == ('shift', None):
        #     return keras.utils.to_categorical([90], num_classes=91)

    # def __generate_outputs(self):
    #     arcs = ['left_arc', 'right_arc']
    #     for arc in arcs:
    #         for relation in dep_relations:
    #             yield tuple([arc, relation])
